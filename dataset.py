...
class MyDataset(Dataset): 
    ...
    def get_train_label(self, index):
        ...
        for i in range(len(anchor_xmin)):
            start_score.append(np.max(ioa_with_temporal_proposals(anchor_xmin[i], anchor_xmax[i], gt_start_periods[:,0], gt_start_periods[:,1])))
            end_score.append(np.max(ioa_with_temporal_proposals(anchor_xmin[i], anchor_xmax[i], gt_end_periods[:,0], gt_end_periods[:,1])))
    ...
    def __getitem__(self, index):  
        ...
        video_name = self.video_list[index]
        video_feature = pd.read_csv(self.feature_path + "csv_mean_" + str(self.temporal_scale) + "/" + video_name + ".csv") 
        ...
        if self.mode == 'train':
            gt_iou_map, start_score, end_score = self.get_train_label(index)
            # the last three elements present: label_confidence, label_start, label_end
            return video_feature, gt_iou_map, start_score, end_score
        else:
            return index, video_feature
